...
flowchart TD
    ...
    subgraph EP2["EP2 · electricalMeterCurrent"]
        ...
        EP2clusters["PowerTopology · ElectricalPowerMeasurement · ElectricalEnergyMeasurement<br/>CommodityMetering · CommodityPrice · ElectricalGridConditions · CommodityTariff (Standard)"]
...
- EP2 (child): `electricalMeter` (0x0514) + `electricalEnergyTariff` (0x0513) + `electricalSensor` (0x0510) — the combination this example is named after, holding the simulated power/energy measurement clusters and exposing the Electrical Energy Tariff clusters directly on this same meter endpoint.
- EP3 (optional child): `electricalEnergyTariff` alone, for the upcoming tariff period.

None of `CommodityTariff`, `CommodityPrice`, `ElectricalGridConditions` (all optional on `electricalEnergyTariff`) or `CommodityMetering` (optional on `electricalMeter`) are required clusters, but EP2 carries all four and EP3 carries `CommodityTariff` — matching the clusters shown on these endpoints in the spec's topology figure. `CommodityMetering` and `CommodityPrice` are set up with `matterbridge`'s own `MatterbridgeCommodityMeteringServer`/`MatterbridgeCommodityPriceServer` (`matterbridge/devices`), `ElectricalGridConditions` with `@matter/node`'s own `ElectricalGridConditionsServer`. `CommodityTariff` goes through `src/flatTariff.ts`, since the day/tariff-component data it publishes (a single flat-rate component covering the whole day, every day — just enough to show the cluster's shape without day/night scheduling) goes beyond `TariffInfo`/`TariffUnit`; it attaches the cluster through `matterbridge`'s own exported `MatterbridgeCommodityTariffServer` (`matterbridge/devices`, alongside `ElectricalUtilityMeter`) rather than a plugin-local subclass, so the `GetTariffComponent`/`GetDayEntry` commands are still implemented without any plugin code. `CommodityMetering.meteredQuantity` is refreshed on the same periodic timer as `ElectricalEnergyMeasurement`.

> [!NOTE]
> The `ElectricalUtilityMeter` device class (`matterbridge/devices`, EP1 above) is new in `matterbridge` 3.10.7. It also ships `addElectricalMeter()`/`addElectricalEnergyTariff()` composition helpers (their `energyTariff` option is what can expose the Electrical Energy Tariff clusters on the same meter endpoint, new in 3.10.8) — this plugin doesn't use them for EP2/EP3, though, because their `CommodityTariff` setup only covers `TariffInfo`/`TariffUnit`, and layering the full day-schedule on top afterward doesn't work: matter.js's `behaviors.require()` silently ignores a second call for an already-present behavior, and overriding via `setAttribute()` instead hits a matter.js model bug when writing `dayEntries` (`DayEntryStruct.randomizationType` is feature-gated `[RNDM]` yet declares a `default: 0`, so the attribute server auto-fills it and then rejects that same fill for failing conformance). A single `behaviors.require()` call with the full data up front sidesteps both issues — this plugin still requires at least matterbridge 3.10.8 (see the `verifyMatterbridgeVersion()` check in `module.ts`), for the `ElectricalUtilityMeter` class itself.
>
...
- The Matter 1.6 "Basic Utility Meter" parent/child endpoint topology (EP1/EP2/EP3), built with the `ElectricalUtilityMeter` single-class device (EP1), `addChildDeviceType()` (EP2/EP3), and semantic tags (`getSemtag()`).
- Simulated power/energy readings refreshed on a timer, to illustrate `setAttribute()` usage on a child endpoint's clusters.
- Minimal flat-rate `CommodityTariff` on EP2/EP3, plus `CommodityPrice`, `CommodityMetering` and `ElectricalGridConditions` on EP2, to illustrate each cluster's shape without a full day/night schedule.
